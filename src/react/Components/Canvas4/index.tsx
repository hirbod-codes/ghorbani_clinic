...
export function Canvas4({ canvasRef, canvasBackground: canvasBackgroundInit, onChange }: CanvasProps) {
    if (!canvasRef)
        canvasRef = useRef()
    ...
    const [canvasInstance, setCanvasInstance] = useState<fabric.Canvas>()

    useEffect(() => {
        if (canvasRef.current) {
            resizeCanvas(canvasRef)
            if (!canvasInstance)
                setCanvasInstance(new fabric.Canvas(canvasRef.current, { isDrawingMode: true }))
        }
    }, [canvasRef.current])
                        ...
                        <AnimatePresence mode="sync">
                            ...
                            {tool === 'pencil' &&
                                <motion.div
                                    ...
                                    style={{ height: '100%', width: '100%', overflow: 'hidden', position: 'absolute' }}
                                >
                                    <PencilTool canvas={canvasInstance} canvasBackground={canvasBackground} />
                                </motion.div>
                            }
