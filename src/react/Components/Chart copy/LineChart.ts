...
export class LineChart extends Shape {
    ...
    animateDefaults(t: DOMHighResTimeStamp, canvasWidth: number, canvasHeight: number, ctx: CanvasRenderingContext2D, hoverEvent?: PointerEvent) {
        this.animate(t, 'verticalLines', canvasWidth, canvasHeight, ctx, (dx, c) => this.strokeVerticalLines(c, getEasingFunction(this.verticalLinesOptions.ease ?? 'easeInSine')(dx)))
        this.animate(t, 'horizontalLines', canvasWidth, canvasHeight, ctx, (dx, c) => this.strokeHorizontalLines(c, getEasingFunction(this.horizontalLinesOptions.ease ?? 'easeInSine')(dx)))
        this.animate(t, 'stroke', canvasWidth, canvasHeight, ctx, (dx, c) => this.stroke(c, getEasingFunction(this.strokeOptions.ease ?? 'easeInSine')(dx)))
        this.animate(t, 'fill', canvasWidth, canvasHeight, ctx, (dx, c) => this.fill(c, getEasingFunction(this.fillOptions.ease ?? 'easeInSine')(dx)))
        this.animate(t, 'hover', canvasWidth, canvasHeight, ctx, (dx, c) => {
            if (!hoverEvent)
                return
            ...
            this.onHover(c, hoverEvent!, index, getEasingFunction(this.hoverOptions.ease ?? 'easeInSine')(dx))
        })
    }
}
