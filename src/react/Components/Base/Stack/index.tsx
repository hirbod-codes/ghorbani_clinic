...
export function Stack({ children, direction = 'horizontal', size = 1, stackProps }: { children?: ReactNode, size?: number, direction?: 'vertical' | 'horizontal', stackProps?: ComponentProps<'div'>, ref?: RefObject<HTMLDivElement> }) {
    ...
    if (local.direction === 'ltr')
        if (direction === 'horizontal')
            return (
                <div {...stackProps} className={cn([`flex flex-row [&>:not(:last-child)]:mr-3`], stackProps?.className)}>
                    {children}
                </div>
            )
        else
        ...
        if (direction === 'horizontal')
            return (
                <div {...stackProps} className={cn([`flex flex-row [&>:not(:first-child)]:mr-3`], stackProps?.className)}>
                    {children}
                </div>
            )
        else
