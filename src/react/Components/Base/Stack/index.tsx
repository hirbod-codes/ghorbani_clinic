...
export function Stack({ children, direction = 'horizontal', size = 1, stackProps, stackRef }: { children?: ReactNode, size?: number, direction?: 'vertical' | 'horizontal', stackProps?: ComponentProps<'div'>, stackRef?: RefObject<HTMLDivElement> }) {
    ...
    if (direction === 'horizontal')
        return (
            <div ref={stackRef} {...stackProps} className={cn([`flex flex-row ltr:[&>:not(:last-child)]:mr-3 rtl:[&>:not(:first-child)]:mr-3`], stackProps?.className)}>
                {children}
            </div>
        )
    else
        return (
            <div ref={stackRef} {...stackProps} className={cn([`flex flex-col [&>:not(:last-child)]:mb-3`], stackProps?.className)}>
                {children}
            </div>
        )
}
